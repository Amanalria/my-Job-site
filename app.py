...
def render_dynamic_homepage_html(raw_html, host, is_alria_mode=False):
    ...
    # ==================== 1. DYNAMIC HEADER BRANDING & TYPOGRAPHY ====================
    hc = settings_obj.get('header_customizer', {})
    tc = settings_obj.get('theme_colors', {})
    
    site_title = hc.get('title_text') or settings_obj.get('site_name') or 'STUDY TOPPER™'
    site_desc = hc.get('desc_text') or settings_obj.get('tagline') or settings_obj.get('domain') or 'Latest Jobs, Results, Etc.'
    
    title_p = soup.find(class_='main-title')
    if title_p:
        a_tag = title_p.find('a')
        if a_tag:
            a_tag.string = site_title
        else:
            title_p.string = site_title
            
    desc_p = soup.find(class_='site-description')
    if desc_p:
        desc_p.string = site_desc
        
    # Dynamic header styles
    hdr_bg = tc.get('header_bg') or hc.get('header_bg') or '#cd0808'
    hdr_txt = tc.get('header_text') or hc.get('title_color') or '#ffffff'
    title_size_d = hc.get('title_size_desktop', 51.0)
    title_size_m = hc.get('title_size_mobile', 36.0)
    desc_size_d = hc.get('desc_size_desktop', 25.8)
    desc_size_m = hc.get('desc_size_mobile', 18.8)
    gap_spacing = hc.get('gap_spacing', 2.0)
    pad_top_d = hc.get('pad_top_desktop', 58)
    pad_top_m = hc.get('pad_top_mobile', 43)
    
    header_style_css = f'''<style id="dynamic-header-styles">
        .site-header {{ background-color: {hdr_bg} !important; }}
        .site-header .inside-header {{ padding: {pad_top_d}px 40px !important; }}
        .main-title a {{ color: {hdr_txt} !important; font-size: {title_size_d}px !important; }}
        .site-description {{ color: {hdr_txt} !important; font-size: {desc_size_d}px !important; margin-top: {gap_spacing}px !important; }}
        @media (max-width: 768px) {{
            .site-header .inside-header {{ padding: {pad_top_m}px 15px !important; }}
            .main-title a {{ font-size: {title_size_m}px !important; }}
            .site-description {{ font-size: {desc_size_m}px !important; }}
        }}
    </style>'''
    
    if soup.find(id='dynamic-header-styles'):
        soup.find(id='dynamic-header-styles').decompose()
    if soup.head:
        soup.head.append(BeautifulSoup(header_style_css, 'html.parser'))

    # Top red banner text
    top_banner_txt = settings_obj.get('top_banner_text')
    if top_banner_txt:
        banner_p = soup.find('p', class_='gb-headline-d55a09d3')
        if banner_p:
            banner_p.clear()
            banner_p.append(BeautifulSoup(f'<strong>{top_banner_txt}</strong>', 'html.parser'))

    # WhatsApp Channel Button
    wa_url = settings_obj.get('socials', {}).get('whatsapp')
    if wa_url:
        wa_a = soup.find('a', class_='whatsapp-btn')
        if wa_a:
            wa_a['href'] = wa_url

    # ==================== 2. INJECT INTO 6 CATEGORY BOXES ====================
    category_column_map = {
        'gb-grid-column-0b76599a': ('result', 'Result', 'result_header_bg'),
        'gb-grid-column-c7488d9a': ('latest-jobs', 'Latest Jobs', 'jobs_header_bg'),
        'gb-grid-column-e64d3148': ('admit-card', 'Admit Card', 'admit_header_bg'),
        'gb-grid-column-d19ddc59': ('answer-key', 'Answer Key', 'answer_header_bg'),
        'gb-grid-column-b48dca36': ('syllabus', 'Syllabus', 'syllabus_header_bg'),
        'gb-grid-column-51daea0e': ('admission', 'Admission', 'admission_header_bg')
    }
    
    grid_headers_cfg = settings_obj.get('grid_headers', {})
    
    for col_cls, (cat_key, def_lbl, col_bg_prop) in category_column_map.items():
        col = soup.find(class_=col_cls)
        if col:
            # Update Column Title & Header Color
            custom_title = grid_headers_cfg.get(cat_key, {}).get('title') if grid_headers_cfg.get(cat_key) else def_lbl
            custom_bg = tc.get(col_bg_prop) or tc.get(f'{cat_key.replace("-", "_")}_header_bg') or '#ab183d'
            
            h_tag = col.find(['p', 'h2'], class_='gb-headline-text')
            if h_tag:
                h_tag.string = custom_title
                h_tag['style'] = f'background-color:{custom_bg} !important;'
    ...
    if top_pages:
        matrix_table = soup.find('table', id='top-pages-matrix-table')
        if matrix_table:
            tds = matrix_table.find_all('td')
            for i, td in enumerate(tds):
                if i < len(top_pages):
                    p_info = top_pages[i]
                    p_txt = p_info.get('text', '').strip()
                    p_url = p_info.get('url', '').strip()
                    if p_txt:
                        td.clear()
                        if p_url:
                            td.append(BeautifulSoup(f'<a href="{p_url}" rel="noopener" style="color:#0000ef; font-weight:700; text-decoration:none;" target="_blank">{p_txt}</a>', 'html.parser'))
                        else:
                            td.string = p_txt

    # ==================== 5. 5 RED INFO SECTIONS ====================
    info_secs = settings_obj.get('info_sections', [])
    if info_secs:
        info_h2_classes = [
            'gb-headline-02a5ae4c', 'gb-headline-4d8c33f9', 'gb-headline-3251e0e2',
            'gb-headline-34d3f76b', 'gb-headline-934edc91'
        ]
        for i, h2_cls in enumerate(info_h2_classes):
            if i < len(info_secs):
                sec = info_secs[i]
                sec_title = sec.get('title', '').strip()
                sec_content = sec.get('content', '').strip()
                
                h2_tag = soup.find(['h2', 'p'], class_=h2_cls)
                if h2_tag:
                    if sec_title:
                        h2_tag.clear()
                        h2_tag.append(BeautifulSoup(f'<strong>{sec_title}</strong>', 'html.parser'))
                    next_p = h2_tag.find_next_sibling('p')
                    if next_p and sec_content:
                        next_p.string = sec_content

    # ==================== 6. FAQS MANAGER ====================
    faqs = settings_obj.get('faq_items', [])
    if faqs:
        faq_container = soup.find('h2', string=re.compile(r'Frequently Asked Questions', re.I))
        if faq_container:
            faq_wrap = faq_container.find_next_sibling('div')
            if faq_wrap:
                faq_wrap.clear()
                for q_idx, f_item in enumerate(faqs[:5]):
                    q_text = f_item.get('q', '').strip()
                    a_text = f_item.get('a', '').strip()
                    if q_text:
                        markup = f'''<p style="margin:12px 0 4px; font-size:14.5px; font-weight:700; color:#a80909;"><span style="color:#000;">Q {q_idx+1}.</span> {q_text}</p>
                        <p style="margin:0 0 14px; font-size:13.5px; line-height:1.6; color:#222; text-align:justify;"><strong style="color:#077822;">Ans.</strong> {a_text}</p>'''
                        faq_wrap.append(BeautifulSoup(markup, 'html.parser'))

    # ==================== 7. FOOTER & SOCIALS ====================
    footer_cfg = settings_obj.get('footer', {})
    if footer_cfg.get('connect_title'):
        c_h3 = soup.find('h3', string=re.compile(r'Connect With Us', re.I))
        if c_h3:
            c_h3.string = footer_cfg.get('connect_title')
            
    soc_links = footer_cfg.get('social_links', [])
    if soc_links:
        sarkari_grid = soup.find('div', class_='sarkari-grid')
        if sarkari_grid:
            sarkari_grid.clear()
            for s in soc_links:
                s_name = s.get('name', '').strip()
    ...
    for a_tag in soup.find_all('a'):
        ...
        if link_text.lower() in ['click here', 'clickhere', 'link 1', 'link 2', 'link', 'download', 'official website', 'apply online', 'view']:
            ...
            if tr:
                ...
                if len(tds) > 0 and tds[0] != a_tag.find_parent(['td', 'th']):
                    context_label = tds[0].get_text().strip()
            if not a_tag.get('aria-label'):
                a_tag['aria-label'] = f"{context_label or 'Official Link'}"
                
    if is_alria_mode:
        soup = inject_alria_live_editor(soup, settings_obj)
